...
function removeFirstOccurrences(str, value) {
  ...
  return str; // ->unchanged str if no value - to remove - passed
}
...
function sumOfCodes(str) {
  ...
  const arr = str.split(''); // ['M', 'y', ' ', ...]
  const sum = arr.reduce((acc, curr) => acc + curr.charCodeAt(), 0);
  return sum;
}
...
function countVowels(str) {
  const arrCount = str.match(/[aeiouy]/gi); // -> if not found length null=>
  if (Array.isArray(arrCount)) {
...
function isPalindrome(str) {
  // console.log(str);// => need to excl ' ', !? & one case
  const strChanged = str.replace(/[\s,!?]/gi, '').toLocaleLowerCase();
  const reversedStr = strChanged.split('').reverse().join('');
  return strChanged === reversedStr;
}
...
function findLongestWord(sentence) {
  // 1st:
  // const arrSplitOnWiteSpace = sentence.split(' ');
  // let longestWord = '';
  // for (let i = 0; i < arrSplitOnWiteSpace.length; i += 1) {
  //   //if word[i] longer than word => update word
  //   if (arrSplitOnWiteSpace[i].length > longestWord.length) {
  //     longestWord = arrSplitOnWiteSpace[i];
  //   }
  // }
  // return longestWord;
  ...
  const arrSplitOnWiteSpace = sentence.split(' ');
  // longest first
  const arrSorted = arrSplitOnWiteSpace.sort((a, b) => b.length - a.length);
  return arrSorted[0];
}
...
function reverseWords(str) {
  ...
  // const arrOfReversedWords = arrOfWords.map((item) =>
  //   item.split('').reverse().join('')
  // );
  // const strFromArr = arrOfReversedWords.join(' ');
  // return strFromArr;
  ...
  for (let i = 0; i < arrOfWords.length; i += 1) {
    const strTemp = arrOfWords[i].split('').reverse().join('');
    arrOfReversedWords.push(strTemp);
  }
...
function encodeToRot13(str) {
  ...
  return str.replace(
    /[a-z]/gi,
    (char) => alphabetOutput[alphabetInput.indexOf(char)]
  );
}
